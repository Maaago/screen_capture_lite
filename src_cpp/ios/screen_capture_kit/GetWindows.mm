...
namespace Screen_Capture
{
    std::vector<Window> GetWindows()
        {
            __block std::vector<Window> windows;
            ...
            @autoreleasepool
            {
                ...
                [SCShareableContent getShareableContentWithCompletionHandler:^(SCShareableContent * _Nullable shareableContent, NSError * _Nullable error) {
                    if (error) {
                        ...
                        {
                            Window window = {
                                .Handle = scWindow.windowID,
                                .Name = "Name set later",
                                .Position = {
                                    .x = static_cast<int>(scWindow.frame.origin.x),
                                    .y = static_cast<int>(scWindow.frame.origin.y),
                                },
                                .Size = {
                                    .x = static_cast<int>(scWindow.frame.size.width),
                                    .y = static_cast<int>(scWindow.frame.size.height),
                                }
                            };

                            size_t nameSize = std::min(sizeof(window.Name)-1, scWindow.title.length);
                            std::memcpy(window.Name, scWindow.title.UTF8String, nameSize);
                            window.Name[nameSize-1] = '\0';
